...
from .models import Music,Myrating
...
def recommend(request):
	...
	if current_user_id>nu:
		music=Music.objects.get(id=15)
		q=Myrating(user=request.user,music=music,rating=0)
		q.save()
	...
	music_list=list(Music.objects.filter(id__in = pred_idxs_sorted,).order_by(preserved)[:10])
	return render(request,'web/recommend.html',{'music_list':music_list})
...
def index(request):
	...
	if query:
		musics = Music.objects.filter(Q(title__icontains=query)).distinct()
		return render(request,'web/list.html',{'musics':musics})
	return render(request,'web/list.html',{'musics':musics})
...
def detail(request,music_id):
	...
	if request.method == "POST":
		...
		ratingObject.music  = musics
		ratingObject.rating = rate
		ratingObject.save()
		messages.success(request,"Your Rating is submited ")
		return redirect("index")
	return render(request,'web/detail.html',{'musics':musics})
